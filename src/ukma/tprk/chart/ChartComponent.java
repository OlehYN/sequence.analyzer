...
public class ChartComponent extends JComponent {
    ...
    protected void paintComponent(final Graphics g) {
        ...
        for (int i = 0; i < units.size(); i++) {
            Shape l = new Rectangle2D.Double(actualPaddingX + i * xCoeff, getHeight() - units.get(i).getValue() * yCoeff - actualPaddingY, xCoeff, units.get(i).getValue() * yCoeff);
            ((Graphics2D) g).draw(l);
        }
        ...
        for (int i = 0; i < yNum; i++) {
            ...
            g.drawString(String.format("%.2f", value / yCoeff), (int) (getWidth() / xNum / 10), (int) (getHeight() - value - actualPaddingY) - (int) (getHeight() / yNum / 10));
        }
        ...
        for (int i = 0; i < xNum; i++) {
            ...
            g.drawString(String.format("%.2f", (value / xCoeff) / units.size()), (int) (actualPaddingX + value) + (int) (getWidth() / xNum / 10), (int) getHeight() - (int) (getHeight() / yNum / 10));
        }
        ...
        if (getMousePosition() != null) {
            ...
            String value = "Частота: " + String.format("%.2f", ((getMousePosition().getX() - actualPaddingX) / xCoeff) / units.size());
            g.drawString(value, getWidth() / 3, getHeight() / 3);

            String value2 = "Кількість: " + String.format("%.2f", ((getHeight() - getMousePosition().getY() - actualPaddingY) / yCoeff));
